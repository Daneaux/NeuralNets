...
namespace MatrixLib
{
    public class AvxMatrix : BaseMatrix, IMatrix
    {
        ...
        public AvxMatrix GetTransposedMatrix()
        {
            ...
            {
                AvxMatrix mt = new AvxMatrix(this.Cols, this.Rows);
        ...
        // fyi: can't find an AVX512 (or any other intrinsic) to do simd logarithm
        public AvxMatrix Log()
        {
            AvxMatrix logMat = new AvxMatrix(this.Cols, this.Rows);
        ...
        // Explicit IMatrix interface implementations
        float[,] IMatrix.Mat => this.Mat;

        IMatrix IMatrix.Add(IMatrix other)
        {
            if (other is AvxMatrix avxOther)
                return this.AddMatrix(avxOther);
            throw new NotSupportedException("Adding non-AVX matrices not supported");
        }

        IMatrix IMatrix.Subtract(IMatrix other)
        {
            if (other is AvxMatrix avxOther)
                return this.SubtractMatrix(avxOther);
            throw new NotSupportedException("Subtracting non-AVX matrices not supported");
        }

        IMatrix IMatrix.Multiply(IMatrix other)
        {
            if (other is AvxMatrix avxOther)
                return this.MatrixTimesMatrix(avxOther);
            throw new NotSupportedException("Multiplying non-AVX matrices not supported");
        }
        ...
        {
            if (vector is AvxColumnVector avxVector)
                return this.MatrixTimesColumn(avxVector);
            throw new NotSupportedException("Multiplying non-AVX column vectors not supported");
        }
        ...
        {
            if (kernel is AvxMatrix avxKernel)
            {
                // Convert AvxMatrix to SquareKernel for convolution
                if (avxKernel.Rows == avxKernel.Cols)
                {
                    SquareKernel squareKernel = new SquareKernel(avxKernel.Mat);
                    return this.Convolution(squareKernel);
                }
            }
            throw new NotSupportedException("Convolution with non-AVX matrices not supported");
        }
        ...
        IMatrix IMatrix.ConvolutionFull(IMatrix kernel)
        {
            if (kernel is AvxMatrix avxKernel)
            {
                ...
                {
                    SquareKernel squareKernel = new SquareKernel(avxKernel.Mat);
                    return this.ConvolutionFull(squareKernel);
                }
            }
            throw new NotSupportedException("Full convolution with non-AVX matrices not supported");
        }

        IMatrix IMatrix.HadamardProduct(IMatrix other)
        {
            if (other is AvxMatrix avxOther)
                return this.HadamardProduct(avxOther);
            throw new NotSupportedException("Hadamard product with non-AVX matrices not supported");
        }

        // Helper method for matrix-vector multiplication
        private IColumnVector MatrixTimesColumn(AvxColumnVector vector)
        {
            ...
            return (IColumnVector)new AvxColumnVector(result);
        }

        // Hadamard product (element-wise multiplication)
        private AvxMatrix HadamardProduct(AvxMatrix other)
        {
